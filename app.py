...
<body>
  ...
  <div id="mapid"></div>
  <p id="coords">Type in the map’s search box above to drop a marker.</p>

  <form id="batch-form" method="post" enctype="multipart/form-data">
    <input type="file" id="file-input" name="file" accept=".csv,.xls,.xlsx"
           onchange="document.getElementById('batch-form').submit()">
    <noscript><button type="submit">Upload & Geocode Batch</button></noscript>
  </form>
...
def index():
    if request.method == "POST":
        ...
        if f.filename.lower().endswith((".xls","xlsx")):
            ...
            dl = f"{name}_geocoded.xlsx"
        else:
            buf.write(df.to_csv(index=False).encode("utf-8"))
            mimetype = "text/csv"
            dl = f"{name}_geocoded.csv"
        buf.seek(0)
        token = f.filename + "_token"
        app.config[token] = (buf, dl, mimetype)
        preview = df.head().to_html(classes="table table-striped", index=False)
        batch_coords = [list(c) for c in coords.tolist()]
